...
(function () {
  ...
  /* ======================================================================
     9. SCRUBBED SCROLL (ASHA-21 — pinned ritual)

     The native path is pure CSS: styles.css drives the pinned ritual with
     animation-timeline: view()/a named view-timeline, gated behind
     @supports (animation-timeline: view()). This file does nothing while
     that gate is open — the effect runs entirely on the compositor and
     needs no JS at all.
     ...
     writes it as inline opacity/transform each frame. It flips
  ...
  function armScrubFallback() {
    ...
    // while the pinned track is actually near the viewport — this is a
    // full-page scroll listener otherwise, and the whole point of the
    // native path is staying off the main thread.
